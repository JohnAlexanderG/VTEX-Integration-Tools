...
TOOLS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "requires_vtex": True,
        "inputs": [
            {"name": "mapping_file", "type": "file", "label": "Maestro referenceCode → skuId", "required": True,
             "accept": ".csv,.xlsx,.json", "position": 0, "role": "input_file"},
            {"name": "references_file", "type": "file", "label": "Referencias objetivo", "required": True,
             "accept": ".csv,.json", "position": 1, "role": "input_file"},
            {"name": "output_prefix", "type": "text", "label": "Prefijo de salida",
             "default": "sku_file_deletion", "flag": "--output-prefix", "role": "output_file"},
            {"name": "mapping_ref_column", "type": "text", "label": "Columna referenceCode del maestro",
             "flag": "--mapping-ref-column"},
            {"name": "mapping_sku_column", "type": "text", "label": "Columna skuId del maestro",
             "flag": "--mapping-sku-column"},
            {"name": "references_column", "type": "text", "label": "Columna referenceCode de referencias",
             "flag": "--references-column"},
            {"name": "limit", "type": "number", "label": "Límite de referencias",
             "flag": "--limit"},
            {"name": "delay", "type": "number", "label": "Delay entre requests (seg)", "default": 1.0,
             "flag": "--delay"},
            {"name": "timeout", "type": "number", "label": "Timeout por request (seg)", "default": 30,
             "flag": "--timeout"},
            {"name": "retries", "type": "number", "label": "Reintentos HTTP 429/5xx", "default": 3,
             "flag": "--retries"},
    ...
    {
        "id": "tool_product_sku_deleter",
        "name": "Eliminar Productos y SKUs VTEX",
        "shortName": "Eliminar Productos/SKUs",
        "description": "Elimina masivamente SKUs (DELETE .../stockkeepingunit/{skuId}) y luego Productos (DELETE .../product/{productId}) en VTEX a partir de un CSV con columnas SkuId/ProductId. Un producto solo se borra si todos sus SKUs fueron eliminados. Soporta modo dry-run.",
        "category": "tools",
        ...
        "inputs": [
            {"name": "input_csv", "type": "file", "label": "CSV con columnas SkuId y ProductId", "required": True,
             "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "confirm_delete", "type": "checkbox", "label": "Confirmar borrado real (requerido si no es dry-run)",
             "flag": "--confirm-delete"},
            {"name": "dry_run", "type": "checkbox", "label": "Simular sin eliminar (dry-run)", "default": True,
             "flag": "--dry-run"},
            {"name": "workers", "type": "number", "label": "Workers concurrentes", "default": 5,
             "flag": "--workers"},
            {"name": "rps", "type": "number", "label": "Requests por segundo", "default": 5.0,
             "flag": "--rps"},
            {"name": "timeout", "type": "number", "label": "Timeout por request (seg)", "default": 30,
             "flag": "--timeout"},
            {"name": "limit", "type": "number", "label": "Límite de filas a procesar (opcional)",
             "flag": "--limit"},
            {"name": "output_dir", "type": "text", "label": "Directorio de salida",
             "default": "product_sku_deletion_output", "flag": "--output-dir", "role": "output_dir"},
        ],
    },
    {
        "id": "tool_price_diff",
        ...
        "inputs": [
            ...
            {"name": "output_prefix", "type": "text", "label": "Prefijo para archivos de salida",
             "default": "price_diff", "position": 3, "role": "output_file"},
    ...
    {
        "id": "step_68",
        "name": "Auditoría de bodegas en NDJSON",
        "shortName": "Auditoría bodegas",
        "description": "Compara el CSV de inventario origen contra el NDJSON generado por el middleware para detectar bodegas y registros que no se están enviando a VTEX.",
        "category": "tools",
        "step": 68,
        "script": "68_inventory_ndjson_audit/inventory_ndjson_audit.py",
        "requires_vtex": False,
        "inputs": [
            {"name": "source_csv", "type": "file",
             "label": "CSV origen (cols: CODIGO SKU, CODIGO SUCURSAL, EXISTENCIA)",
             "required": True, "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "ndjson_file", "type": "file",
             "label": "NDJSON generado por el middleware",
             "required": True, "accept": ".ndjson,.jsonl", "position": 1, "role": "input_file"},
            {"name": "output_prefix", "type": "text",
             "label": "Prefijo de archivos de salida",
             "default": "bodega_audit", "position": 2, "role": "output_file"},
            {"name": "sku_column", "type": "text", "label": "Columna SKU (CSV)",
             "default": "CODIGO SKU", "flag": "--sku-column"},
            {"name": "warehouse_column", "type": "text", "label": "Columna bodega (CSV)",
             "default": "CODIGO SUCURSAL", "flag": "--warehouse-column"},
            {"name": "quantity_column", "type": "text", "label": "Columna cantidad (CSV)",
             "default": "EXISTENCIA", "flag": "--quantity-column"},
            {"name": "dry_run", "type": "checkbox",
             "label": "Dry run (analizar sin escribir archivos de salida)",
             "flag": "--dry-run"},
            {"name": "verbose", "type": "checkbox",
    ...
    {
        "id": "step_69",
        "name": "Exportar Master Data (scroll)",
        "shortName": "Exportar Master Data",
        "description": "Consulta VTEX Master Data (dataentities/scroll) con autopaginación y exporta todos los registros de una entidad a CSV + reporte Markdown.",
        "category": "tools",
        "step": 69,
        "script": "69_vtex_masterdata_search_exporter/vtex_masterdata_search_exporter.py",
        "requires_vtex": True,
        "inputs": [
            {"name": "data_entity_name", "type": "text", "label": "Data Entity (ej. CL)",
             "required": True, "default": "CL", "flag": "--data-entity-name"},
            {"name": "output", "type": "text", "label": "Archivo CSV de salida",
             "default": "export.csv", "flag": "--output", "role": "output_file"},
            {"name": "report", "type": "text", "label": "Reporte Markdown",
            ...
            {"name": "schema", "type": "text",
             "label": "Nombre del schema (_schema, solo si la cuenta lo exige)",
             "flag": "--schema"},
            {"name": "delay", "type": "number", "label": "Delay entre páginas (seg)",
             "default": 0.5, "flag": "--delay"},
            {"name": "timeout", "type": "number", "label": "Timeout por request (seg)",
             "default": 30, "flag": "--timeout"},
            {"name": "retries", "type": "number", "label": "Reintentos (429/5xx/timeout)",
             "default": 3, "flag": "--retries"},
        ],
    },
    {
        "id": "step_70",
        "name": "Limpiar y formatear datos personales (CO)",
        "shortName": "Datos personales CO",
        "description": "Limpia, valida y formatea un CSV de datos personales de clientes colombianos (cédula, celular, fecha de nacimiento). Filas inválidas van a un CSV de revisión aparte.",
        "category": "tools",
        "step": 70,
        "script": "70_csv_datos_personales_co/csv_datos_personales_co.py",
        "requires_vtex": False,
        "inputs": [
            {"name": "input_csv", "type": "file",
             "label": "CSV de datos personales (Nombres, Apellidos, Cedula, Celular, Fecha Nacimiento, Correo, Genero, Ciudad, Tipo Documento, Sucursal)",
             "required": True, "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "output_prefix", "type": "text", "label": "Prefijo de archivos de salida",
             "default": "resultado", "position": 1, "role": "output_file"},
            {"name": "encoding", "type": "text", "label": "Encoding CSV",
             "default": "utf-8", "flag": "--encoding"},
        ],
    },
    {
        "id": "step_71",
        "name": "Comparar correos vs Master Data",
        "shortName": "Diff correos Master Data",
        "description": "Compara un CSV de datos nuevos contra un export de Master Data por correo (case-insensitive). Clasifica en coincidencias a actualizar, sin cambios, y a crear, armando patch_payload/create_payload listos para los pasos de PATCH/POST.",
        "category": "tools",
        "step": 71,
        "script": "71_email_masterdata_diff/email_masterdata_diff.py",
        "requires_vtex": False,
        "inputs": [
            {"name": "archivo1", "type": "file",
             "label": "Export actual de Master Data (.csv, requiere columnas id + email)",
             "required": True, "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "archivo2", "type": "file",
             "label": "Datos nuevos a evaluar (.csv, ej. salida de 'Datos personales CO')",
             "required": True, "accept": ".csv", "position": 1, "role": "input_file"},
            {"name": "output_prefix", "type": "text", "label": "Prefijo de archivos de salida",
             "default": "resultado", "position": 2, "role": "output_file"},
            {"name": "email_col1", "type": "text", "label": "Columna de correo en archivo1",
             "default": "email", "flag": "--email-col1"},
            {"name": "email_col2", "type": "text", "label": "Columna de correo en archivo2",
             "default": "Correo", "flag": "--email-col2"},
            {"name": "encoding", "type": "text", "label": "Encoding CSV",
             "default": "utf-8", "flag": "--encoding"},
        ],
    },
    {
        "id": "step_72",
        "name": "Actualizar Master Data (PATCH)",
        "shortName": "PATCH Master Data",
        "description": "Ejecuta PATCH real contra VTEX Master Data para actualizar documentos existentes, a partir de un CSV con columnas id y patch_payload (salida de 'Comparar correos vs Master Data'). Por defecto ejecuta en dry-run.",
        "category": "tools",
        "step": 72,
        "script": "72_vtex_masterdata_patch_updater/vtex_masterdata_patch_updater.py",
        "requires_vtex": True,
        "inputs": [
            {"name": "input_csv", "type": "file",
             "label": "CSV con columnas id y patch_payload",
             "required": True, "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "data_entity_name", "type": "text", "label": "Data Entity",
             "default": "CL", "flag": "--data-entity-name"},
            {"name": "id_col", "type": "text", "label": "Columna con el id del documento",
             "default": "id", "flag": "--id-col"},
            {"name": "payload_col", "type": "text", "label": "Columna con el JSON del patch",
             "default": "patch_payload", "flag": "--payload-col"},
            {"name": "dry_run", "type": "checkbox",
             "label": "Dry run (simular sin hacer PATCH reales)",
             "default": True, "flag": "--dry-run"},
            {"name": "delay", "type": "number", "label": "Delay entre requests (seg)",
             "default": 0.5, "flag": "--delay"},
            {"name": "timeout", "type": "number", "label": "Timeout por request (seg)",
             "default": 30, "flag": "--timeout"},
            {"name": "retries", "type": "number", "label": "Reintentos (429/5xx/timeout)",
            ...
            {"name": "encoding", "type": "text", "label": "Encoding CSV",
             "default": "utf-8", "flag": "--encoding"},
            {"name": "output", "type": "text", "label": "CSV de errores",
             "default": "errores.csv", "flag": "--output", "role": "output_file"},
            {"name": "report", "type": "text", "label": "Reporte Markdown",
             "default": "reporte.md", "flag": "--report", "role": "output_file"},
            {"name": "log", "type": "text", "label": "Log incremental (progreso fila por fila)",
             "default": "log.csv", "flag": "--log", "role": "output_file"},
        ],
    },
    {
        "id": "step_73",
        "name": "Crear documentos Master Data (POST)",
        "shortName": "POST Master Data",
        "description": "Ejecuta POST real contra VTEX Master Data para crear documentos nuevos, a partir de un CSV con columna create_payload (salida de 'Comparar correos vs Master Data'). Por defecto ejecuta en dry-run. Cuidado: re-ejecutar sobre filas ya creadas genera duplicados.",
        "category": "tools",
        "step": 73,
        "script": "73_vtex_masterdata_document_creator/vtex_masterdata_document_creator.py",
        "requires_vtex": True,
        "inputs": [
            {"name": "input_csv", "type": "file",
             "label": "CSV con columna create_payload",
             "required": True, "accept": ".csv", "position": 0, "role": "input_file"},
            {"name": "data_entity_name", "type": "text", "label": "Data Entity",
             "default": "CL", "flag": "--data-entity-name"},
            {"name": "payload_col", "type": "text", "label": "Columna con el JSON del documento a crear",
             "default": "create_payload", "flag": "--payload-col"},
            {"name": "dry_run", "type": "checkbox",
             "label": "Dry run (simular sin hacer POST reales)",
             "default": True, "flag": "--dry-run"},
            {"name": "delay", "type": "number", "label": "Delay entre requests (seg)",
             "default": 0.5, "flag": "--delay"},
            {"name": "timeout", "type": "number", "label": "Timeout por request (seg)",
