...
export default function LogPanel({ logs, className = '' }: Props) {
  ...
  if (logs.length === 0) {
    return (
      <div className={`scrollbar-thin bg-surface-0 rounded-lg p-4 font-mono text-xs text-ink-4 ${className}`}>
        Sin logs aún…
      </div>
    )
  }
  ...
  return (
    <div className={`scrollbar-thin bg-surface-0 rounded-lg p-4 font-mono text-xs overflow-y-auto ${className}`}>
      {logs.map((entry, i) => {
